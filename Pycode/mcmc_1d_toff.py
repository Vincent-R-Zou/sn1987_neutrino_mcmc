...
# =========================
    ...
    percision: int = 32   
    steps, dicards, thin = 100000, 80000, 20
...
def log_likelihood_K(theta):
    ...
    if kx:
        ...
        t_off_K = args_list[PARAMS_ALL.index('t_off_K')]
        phys_args = args_list[:8] 
        
        t_K_phys = t_K + t_off_K

        fun1_K = lambda x: SignalRate_1d.SR_K(t_K_phys, x, c_K, *phys_args) * Errors.Error_E(x, E_K, dE_K)

        part1_K = df.gl3_integrate(SignalRate_1d.SR_K, ranges_K, percision, args=phys_args)
...
def log_likelihood_I(theta):
    ...
    if ix:
        ...
        t_off_I = args_list[PARAMS_ALL.index('t_off_I')]

        phys_args = args_list[:8]
        
        t_I_phys = t_I + t_off_I

        fun1_I = lambda x: SignalRate_1d.SR_I(t_I_phys, x, c_I, *phys_args)*Errors.Error_E(x, E_I, dE_I)

        fun2_I = lambda x, c, t: SignalRate_1d.SR_I(t, x, c, *phys_args)*Errors.Error_E(x, E_I, dE_I)

        part1_I = df.gl3_integrate(SignalRate_1d.SR_I, ranges_I, percision, args=phys_args)

        part2_I = np.clip(df.gl_integrate(fun1_I, E_min_I , E_max), 1e-100, None)

        part3_I = B_I/2

        part4_I= df.gl2_integrate_vec(fun2_I, ((E_min_I, E_max), (-1,1)), t_I_phys, percision)
...
def log_likelihood_B(theta):
    ...
    if bx:
        ...
        t_off_B = args_list[PARAMS_ALL.index('t_off_B')]

        phys_args = args_list[:8]
        
        t_B_phys = t_B + t_off_B

        fun1_B = lambda x: SignalRate_1d.SR_B(t_B_phys, x, c_B, *phys_args)*Errors.Error_E(x, E_B, dE_B)

        part1_B = df.gl3_integrate(SignalRate_1d.SR_B, ranges_B, percision, args=phys_args)
